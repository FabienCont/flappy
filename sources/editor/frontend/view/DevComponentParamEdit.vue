<template>
  <div class="dev-component-param">
    ...
    <div class="dev-component-param-value">
      <dev-select @input="(val)=>updateType(val)" v-if="value['_type']" label="type" :border="false" :default="value['_type']" :options="componentParamTypes">type :</dev-select>
      <div>
        <template v-if="value['_default'] !== undefined" >
          <div class="dev-component-default-checkbox">
            <dev-checkbox :val="setToFalse" v-model="selectedCheckbox" label="Set to false"></dev-checkbox>
            <dev-checkbox :val="setSnippet" v-model="selectedCheckbox" label="default is snippet"></dev-checkbox>
          </div>
          <dev-input v-if="value['_type']==='string' || value['_type']==='number'" name="default" :border="false" :type="typeof value['_default'] === 'number' ?'number':'string'" @update:inputValue="newVal=>updateDefault(newVal)" :isEditable="true" :inputValue="value['_default']"></dev-input>
          <span v-else-if="isSetToFalse">{{value['_default']}}</span>
          <!-- <dev-select v-else-if="value['_type']==='snippet'" @input="(val)=>this.value['_type']=val" v-if="value['_type']" label="type" :border="false" :default="value['_type']" :options="componentParamTypes">type :</dev-select> -->
...
export default {
  ...
  watch:{
    value:{
      handler(val){
        ...
        }else if(val._type!=='dico' && val._dico ){
  ...
  methods:{
    ...
    deleteParam:function(){
      this.$emit('del-param',{path:[this.name]})
    },
    childDeleteParam:function(path){
      this.$emit('del-param',{path:[this.name,...path]})
    },
    ...
    addDicoElem:function(){
      this.updateDicoDef({'newParam':{
        '_type':'number',
        'default':1
      }})
    },
    addObjectElem:function(){
      this.updateObjectDef({'newParam':{
        '_type':'number',
        'default':1
      }})
    },
