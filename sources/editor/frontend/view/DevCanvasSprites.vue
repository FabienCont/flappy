...
export default {
  ...
  mounted(){
    ...
    this.theatreInstance = new Theatre({
      ...
      expose: false,
      sharp: true,
      scenarioCtx: require.context('debug/theatre/previewSprites/lifecycles/', true, /^\.\/scenario\.json$/, 'sync'),
      hooksCtx:require.context('debug/theatre/previewSprites/lifecycles/', true, /\.\/(\w+)\/(\w+)\.js$/, 'sync'),
      assetsCtx:{},
